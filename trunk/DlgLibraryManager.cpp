...
void CDlgLibraryManager::DoDataExchange(CDataExchange* pDX)
{
	...
	if( !pDX->m_bSaveAndValidate )
	{
		...
		for( int i=0; i<m_footlib->GetNumLibs(); i++ )
			m_combo_libfile.InsertString( i, *m_footlib->GetLibraryFilename(i) );
...
void CDlgLibraryManager::OnBnClickedButtonMakePdf()
{
	...
	for( ilib=first_ilib; ilib<=last_ilib; ilib++ )
	{
		...
		for( int ih=0; ih<m_footlib->GetNumHeadings( ilib ); ih++ )
		{
			for( int i=0; i<m_footlib->GetNumFootprints(ilib, ih) ; i++ )
			{
				// get next footprint
				CShape foot;
				int err = foot.MakeFromFile( NULL, *m_footlib->GetFootprintName( ilib, ih, i ), 
					*m_footlib->GetLibraryFilename( ilib ), m_footlib->GetFootprintOffset( ilib, ih, i ) );
				if( err )
					ASSERT(0);
